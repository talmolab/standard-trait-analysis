...
@app.cell
def _(logging):
    TRAITS_CSV_PATH = "tests/data/traits_summary.csv"
    EXPERIMENT_EXCEL_PATH = "tests/data/Wheat_EDPIE_cylinder_master_data_df.xlsx"
    LOGGING_LEVEL = logging.DEBUG
    return TRAITS_CSV_PATH, EXPERIMENT_EXCEL_PATH

@app.cell
def _(logging, LOGGING_LEVEL):
    

@app.cell
def _(os, datetime, Path):
...
def _(pd, merged_data):
    ...
    merged_data['plant_age_days'] = pd.to_numeric(merged_data['plant_age_days'], errors='coerce')
...
def _(Path, merged_data, top_dir):
    ...
    sleap_root_traits_7d.to_csv(Path(top_dir) / "biomass_and_sleap_root_traits_7d_after_qc.csv", index=False)
    print(f"Saved 7 Day-Old Plants data to CSV file: {Path(top_dir) / 'sleap_root_traits_7d.csv'} with shape: {sleap_root_traits_7d.shape}")

    # 14 Day-Old Plants only
    sleap_root_traits_14d = merged_data[merged_data['plant_age_days'] == 14]

    # Save the filtered DataFrame to a CSV file
    sleap_root_traits_14d.to_csv(Path(top_dir) / "biomass_and_sleap_root_traits_14d_after_qc.csv", index=False)
    print(f"Saved 14 Day-Old Plants data to CSV file: {Path(top_dir) / 'sleap_root_traits_14d.csv'} with shape: {sleap_root_traits_14d.shape}")
