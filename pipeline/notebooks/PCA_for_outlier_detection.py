...
def _(df, plt, sns, sub_dir):
    ...
    _ax = sns.countplot(y="Geno", data=df, order=df["Geno"].value_counts().index)
    plt.title("")
    _ax.xaxis.grid(True)
    _ax.set(xlabel="Geno")
    sns.despine(trim=True, left=True)
    plt.savefig(
        f"{sub_dir}/line_counts_no_nans.png", bbox_inches="tight", facecolor="white"
    )
    plt.show()
...
def _(COL_START, df, np, plt, sns, sub_dir):
    ...
    plt.savefig(
        f"{sub_dir}/corr_table.png", bbox_inches="tight", facecolor="white", dpi=600
    )
    plt.show()
...
def _(pca_95, pca_n):
    print(
        f"Variance explained by all {pca_n} principal components = ",
...
def _(np, pca_95, plt, sub_dir):
    ...
    plt.savefig(
        f"{sub_dir}/pca_95_elbow_plot_per_plant.png",
        bbox_inches="tight",
        facecolor="white",
    )
    plt.show()
...
def _(Z_pca_95, plt, sub_dir):
    ...
    plt.savefig(
        f"{sub_dir}/transformed_data_95percent.png",
        bbox_inches="tight",
        facecolor="white",
    )
    plt.show()
...
def _(df_new, mpl, pca_95_cumsum, pca_95_explained, plt, sub_dir):
    ...
    plt.title(
        f"Transformed data by the principle components ({pca_95_cumsum[2]:.4f}% variability)",
        pad=15,
    )
    plt.savefig(
        f"{sub_dir}/2d_scatterplot_pc1_pc2.png", bbox_inches="tight", facecolor="white"
    )
    plt.show()
...
def _(COL_START, df, np, pca_95, plt, sub_dir):
    ...
    plt.clim(
        -np.abs(pca_95.components_[:5]).max(), np.abs(pca_95.components_[:5]).max()
    )
    ...
    plt.savefig(
        f"{sub_dir}/features_pca5_colorbar.png", bbox_inches="tight", facecolor="white"
    )
...
def _(df_new, pca_95_cumsum, pca_95_explained, plt, sub_dir):
    ...
    plt.savefig(
        f"{sub_dir}/2d_scatterplot_pc1_pc2_aspect_ratio_equal.png",
        dpi=120,
        bbox_inches="tight",
        facecolor="white",
    )
...
def _(df_new, pca_95_cumsum, pca_95_explained, plt, sns, sub_dir):
    ...
    joint = sns.jointplot(
        data=data, x="PC1", y="PC2", kind="hist", cmap="mako", cbar="True"
    )
    sns.kdeplot(data=data, x="PC1", y="PC2", levels=5, color="w", linewidths=1)
    # JointGrid has a convenience function
    joint.set_axis_labels(
        f"PC1: {pca_95_explained[0]:.4f}% variability",
        f"PC2: {pca_95_explained[1]:.4f}% variability",
        fontsize=16,
    )
    joint.fig.suptitle(
        f"Transformed data by the principle components ({pca_95_cumsum[1]:.4f}% variability)"
    )
    joint.fig.tight_layout()
    plt.savefig(
        f"{sub_dir}/2d_scatterplot_pc1_pc2_heatmap.png",
        dpi=120,
        bbox_inches="tight",
        facecolor="white",
    )
...
def _(df_new, pca_95_cumsum, pca_95_explained, plt, sns, sub_dir):
    ...
    sns.lmplot(data=df_new, x="PC1", y="PC2", fit_reg=False, hue="Geno")
    ...
    plt.savefig(
        f"{sub_dir}/2d_scatterplot_pc1_pc2_lines.png",
        bbox_inches="tight",
        facecolor="white",
    )
    plt.show()
...
def _(pca_95_cumsum):
    ...
    print(
        f" {var_idx + 1} principal components account for {var} % of variability  {var}"
    )
...
def _(df_new, pca_95_cumsum, pca_95_explained, plt, sns, sub_dir):
    ...
    plt.savefig(
        f"{sub_dir}/2d_scatterplot_pc1_pc2_mdist_contour.png",
        bbox_inches="tight",
        facecolor="white",
    )
    plt.show()
